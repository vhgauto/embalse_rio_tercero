d_temp <- read_csv("datos/d.csv", show_col_types = FALSE) |>
  filter(param == "temperatura") |>
...
#   arrange(fecha) |>
#   filter(between(fecha, fecha_i, fecha_f)) |>
#   mutate(fecha_min = lag(fecha, default = min(d_temp$fecha))) |>
#   mutate(fecha_max = if_else(fecha == min(fecha), min(d_temp$fecha), fecha)) |>
...
g_temp <- ggplot(d_temp, aes(fecha, valor)) +
  geom_rect(
    data = d_est,
    aes(
      xmin = fecha_min,
  ...
  labs(y = "Temperatura del agua (°C)", x = NULL) +
