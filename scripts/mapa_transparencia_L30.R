...
mod_tbl <- terra::extract(r_aguaX, p) |>
  as_tibble() |>
  select(-fmask, -ID) |>
  mutate(ds = p$valor, .before = 1) |>
  mutate(ratio = B06 / B05) |>
  select(ds, ratio)
...
# ráster a tbl
r_agua_tbl <- terra::as.data.frame(r_aguaX) |>
  as_tibble() |>

  # filter(nir != 0) |>

  transmute(ratio = B06 / B05) # <-------- cociente de bandas
...
predict_ds <- terra::as.data.frame(r_aguaX$B07, xy = TRUE) |>
  as_tibble() |>
  # dplyr::filter(nir != 0) |>
  bind_cols(pred_tbl) |>
  select(-B07) |>
...
fecha_transparencia <- ymd(sub("_temp.tif", "", basename(sources(r_aguaX))))
