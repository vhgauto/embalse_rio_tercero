...
g_ciano_mensual <- ggplot(d_micro, aes(fecha, suma, fill = algas)) +
  ...
  geom_line(
    data = d_micro_suma, aes(fecha, s), inherit.aes = FALSE, color = "black",
    linewidth = .5
  ) +
  geom_point(
    data = d_micro_suma, aes(fecha, s), inherit.aes = FALSE, size = 2,
    shape = 21, fill = "white", color = "black", stroke = .7
  ) +
  ...
  scale_y_continuous(
    ...
    breaks = scales::breaks_width(1e6),
    labels = \(Q) {
      if_else(
        Q == 0,
        "0",
