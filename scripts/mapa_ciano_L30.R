...
ciano_tbl <- terra::extract(r_aguaX, p) |>
  as_tibble() |>
  select(-fmask, -ID)
...
predict_ciano <- terra::as.data.frame(r_aguaX$B07, xy = TRUE) |>
  as_tibble() |>
  bind_cols(pred_tbl) |>
  select(-B07) |>
...
ggplot() +
  ...
  coord_cartesian(xlim = c(0, tr * 15)) +
...
fecha_ciano <- ymd(sub("_temp.tif", "", basename(sources(r_aguaX))))
