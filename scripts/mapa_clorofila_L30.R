...
cla_tbl <- terra::extract(r_aguaX, p) |>
  as_tibble() |>
  select(-fmask, -ID)
...
mod_tbl <- terra::extract(r_aguaX, p) |>
  ...
  select(-fmask, -ID) |>
  mutate(cla = p$valor, .before = 1) |>
  mutate(ratio = B07 / B05) |> # <-------- cociente de bandas
  select(cla, ratio)
...
predict_cla <- terra::as.data.frame(r_aguaX$B07, xy = TRUE) |>
  as_tibble() |>
...
fecha_clorofila <- ymd(sub("_temp.tif", "", basename(sources(r_aguaX))))
